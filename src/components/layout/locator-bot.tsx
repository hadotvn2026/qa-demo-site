...
function rateLocator(loc: string): number {
  ...
  if (loc.startsWith("getByRole") || loc.startsWith("getByLabel") || loc.startsWith("getByText")) return 5;
  if (loc.startsWith("getByTestId") || loc.startsWith("getByPlaceholder")) return 4;
  if (/data-testid|aria-label/i.test(loc)) return 4;
  if (loc.startsWith("locator('#")) return 3;
  if (loc.startsWith("locator('.")) return 3;
  if (/(@id\s*=|\[id=)/.test(loc)) return 3;
  if (/(^|[^\w-])#[a-z][\w-]*/i.test(loc)) return 3;
  if (/(@name\s*=|\[name=)/.test(loc)) return 3;
  if (/(@role\s*=|\[role=)/.test(loc)) return 3;
  if (/^\.[a-z]/i.test(loc) || /^[a-z]+\.[a-z]/i.test(loc)) return 3;
  if (/(@(href|for)\s*=|\[(href|for)=)/.test(loc)) return 2;
  if (/(@(type|alt|title|placeholder)\s*=|\[(type|alt|title|placeholder)=)/.test(loc)) return 2;
  if (/(contains\(text\(\)|:contains\(|normalize-space\(\)\s*=|text\(\)\s*=)/.test(loc)) {
    return /^\/\/\*/.test(loc) ? 2 : 2;
  }
  if (/:(disabled|checked|enabled)/.test(loc)) return 2;
  if (/:nth-(of-type|child|last-child|last-of-type)/.test(loc)) return 1;
  if (/^\/\/[a-z]+$/.test(loc) || /^[a-z]+$/.test(loc)) return 1;
  if (/>/.test(loc) || /\s+/.test(loc)) return 1;
  return 2;
}
...
function getAccessibleName(el: Element): string | null {
  ...
  // Fallback to name attribute if label is missing
  const nameAttr = el.getAttribute("name");
  if (nameAttr) {
    // Capitalize first letter to match common label styles
...
function buildPlaywrightCandidates(
  tag: string,
  text: string,
  attrs: Array<{ name: string; value: string }>,
  el: Element
  ...
  if (testId) pw.push(`getByTestId('${testId}')`);

  const ariaLabel = getAttr("aria-label");
  if (ariaLabel) pw.push(`getByLabel('${ariaLabel.replace(/'/g, "\\'")}')`);

  let role = getAttr("role");
  if (!role) {
    if (tag === "button" || (tag === "input" && ["button", "submit", "reset"].includes(getAttr("type") || ""))) role = "button";
    else if (tag === "a" && getAttr("href")) role = "link";
    else if (/^h[1-6]$/.test(tag)) role = "heading";
    else if (tag === "input" && getAttr("type") === "checkbox") role = "checkbox";
    else if (tag === "input" && getAttr("type") === "radio") role = "radio";
  ...
  if (role) {
    ...
    if (name) {
      ...
      pw.push(`getByRole('${role}', { name: '${cleanName}' })`);
    } else {
      pw.push(`getByRole('${role}')`);
    }
  }

  const placeholder = getAttr("placeholder");
  if (placeholder) {
    pw.push(`getByPlaceholder('${placeholder.replace(/'/g, "\\'")}', { exact: true })`);
  }

  if (text && text.length <= 80) {
    pw.push(`getByText('${text.replace(/'/g, "\\'")}', { exact: true })`);
  }

  const id = getAttr("id");
  if (id) pw.push(`locator('#${CSS.escape(id)}')`);

  const cls = getAttr("class");
  if (cls) {
    const firstClass = cls.split(/\s+/).filter(Boolean)[0];
    if (firstClass) pw.push(`locator('.${CSS.escape(firstClass)}')`);
  }

  return Array.from(new Set(pw));
}
...
function matchCheatsheetRecipes(el: Element, text: string): CheatsheetMatch[] {
  ...
  matches.push({
    ...
    playwright: `locator('${tag}')`,
    xpath: `//${tag}`,
    css: tag,
  });

  if (tag === "img") {
    matches.push({ recipe: "Image element", playwright: "getByRole('img')", xpath: "//img", css: "img" });
  }
  if (tag === "a") {
    matches.push({ recipe: "Link element", playwright: "getByRole('link')", xpath: "//a", css: "a" });
  }

  if (id) {
    matches.push({
      recipe: "Element with id I",
      playwright: `locator('#${CSS.escape(id)}')`,
      xpath: `//*[@id=${xpathLiteral(id)}]`,
      css: `#${CSS.escape(id)}`,
    });
    matches.push({
      recipe: "Element <E> with id I",
      playwright: `locator('${tag}#${CSS.escape(id)}')`,
  ...
  for (const [a, v] of labeledAttrs) {
    ...
    let pwStr = `locator('${tag}[${a}=${cssAttr(v)}]')`;
    if (a === "data-testid") pwStr = `getByTestId('${v}')`;
    else if (a === "placeholder") pwStr = `getByPlaceholder('${v}')`;
    else if (a === "aria-label") pwStr = `getByLabel('${v}')`;
  ...
  if (tag === "a" && href) {
    matches.push({
      ...
      playwright: `getByRole('link').filter({ has: page.locator('[href=${cssAttr(href)}]') })`,
  ...
  if (text) {
    matches.push({
      ...
      playwright: `getByText('${text.replace(/'/g, "\\'")}', { exact: true })`,
      xpath: `//*[normalize-space()=${xpathLiteral(text)}]`,
      css: null,
    });
    matches.push({
      recipe: "Element <E> containing text 't'",
      playwright: `locator('${tag}').filter({ hasText: '${text.replace(/'/g, "\\'")}' })`,
    ...
    if (tag === "a") {
      matches.push({
        ...
        playwright: `getByRole('link', { name: '${text.replace(/'/g, "\\'")}', exact: true })`,
  ...
  if (isDisabled) {
    matches.push({
      ...
      playwright: `locator('${tag}').filter({ disabled: true })`,
      xpath: `//${tag}[@disabled]`,
      css: `${tag}:disabled`,
    });
  }
  ...
  if (isChecked) {
    matches.push({
      ...
      playwright: `locator('*').filter({ checked: true })`,
      xpath: "//*[@checked]",
      css: "*:checked",
    });
  }
  ...
  return matches.map(m => {
    let selenium = null;
    if (m.css) {
       if (m.css.startsWith('#') && !m.css.includes(' ') && !m.css.includes('[')) selenium = `By.id("${m.css.slice(1).replace(/\\/g, '')}")`;
       else if (/^[a-z]+$/.test(m.css)) selenium = `By.tagName("${m.css}")`;
       else selenium = `By.cssSelector("${m.css.replace(/"/g, '\\"')}")`;
    } else if (m.xpath) {
       selenium = `By.xpath("${m.xpath.replace(/"/g, '\\"')}")`;
    }
    return { ...m, selenium };
  });
}
...
function analyzeElement(el: Element): ElementAnalysis {
  ...
  const playwright = buildPlaywrightCandidates(tag, text, attrs, el);
...
function BrainstormPanel({ analysis, stage, copied, onCopy }: BrainstormPanelProps) {
  const thinking = analysis !== null && stage < 7;
    ...
    <div className="rounded-2xl border border-border bg-slate-950/40 p-4">
      ...
      {analysis === null ? (
        ...
        <div className="space-y-2 font-mono text-sm">
          ...
          {stage >= 6 && (
            <CandidateList
              label="Possible CSS selectors (Stable to Fragile)"
          ...
          {stage >= 7 && (
            <CandidateList
              label="Possible XPath selectors (Use as last resort)"
          ...
          <table className="w-full border-collapse text-left text-xs">
            <thead className="text-[0.7rem] uppercase tracking-[0.16em] text-muted-foreground">
              <tr>
                ...
                <th className="px-2 py-2 font-medium text-emerald-300">Playwright</th>
                <th className="px-2 py-2 font-medium text-amber-300">Selenium</th>
                <th className="px-2 py-2 font-medium text-sky-300">CSS</th>
                <th className="px-2 py-2 font-medium text-violet-300">XPath</th>
              </tr>
            </thead>
            <tbody>
              {[...matches]
                .map((m) => ({
                  ...m,
                  score: Math.max(
                    m.playwright ? rateLocator(m.playwright) : 0,
                    m.selenium ? rateLocator(m.selenium) : 0,
                ...
                .map((m, i) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-2">
                      <CheatsheetCell
                        value={m.playwright ?? null}
                        accent="text-emerald-300"
                        copied={copied === m.playwright}
                        onCopy={onCopy}
                      />
                    </td>
                    <td className="px-2 py-2">
                      <CheatsheetCell
                        value={m.selenium ?? null}
                        accent="text-amber-300"
                        copied={copied === m.selenium}
                        onCopy={onCopy}
                      />
                    </td>
                    <td className="px-2 py-2">
